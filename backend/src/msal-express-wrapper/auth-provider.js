...
class AuthProvider {
                ...
                case constants.AppStages.SIGN_IN: {
                    ...
                    try {
                        ...
                        if (this.tokenValidator.validateIdToken(tokenResponse.idTokenClaims)) {
                            ...
                            // If frontend provided a returnTo URL, redirect back there. Otherwise use homePageRoute
    ...
    getToken = async (req, res, next) => {
        ...
        const resourcePath = this.normalizeResourcePath(req.route.path);
        const resource = Object.values(this.appSettings.resources)
            .find((resource) => resource.callingPageRoute === resourcePath);

        if (!resource) {
            console.log(`MSAL getToken: no resource mapping found for path ${req.route.path}`);
            return res.status(400).send('Invalid resource path');
        }

        const scopes = resource.scopes;
        const resourceName = this.getResourceName(resourcePath);
    ...
    getResourceName = (path) => {
        ...
        const index = Object.values(this.appSettings.resources).findIndex((resource) => resource.callingPageRoute === normalizedPath);
