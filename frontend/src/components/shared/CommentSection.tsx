...
const renderWithMentions = (text: string) =>
  text.split(/(@\w+)/g).map((part, i) =>
    part.startsWith('@') ? (
      <span key={i} className="text-[#F15A22] font-medium">
        {part}
      </span>
    ) : (
      ...
      <form onSubmit={handleSubmit} className="px-5 py-3 border-t border-slate-50 flex items-end gap-2">
        ...
        <div className="flex-1 relative">
          <MentionInput
            ...
            rows={1}
            className="w-full px-3 py-2 text-sm rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#F15A22]/20 focus:border-[#F15A22] transition-all resize-none placeholder:text-slate-400"
          />
        </div>
        <button
          type="submit"
