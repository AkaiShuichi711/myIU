      ...
      {items.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          ...
          {canAddMore && (
            <div
              {...getRootProps()}
              className="h-28 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-600 flex flex-col items-center justify-center gap-1.5 cursor-pointer hover:border-[#1e51f9] hover:bg-[#1e51f9]/4 transition-all"
            >
              <input {...getInputProps()} />
      ...
      {items.length === 0 && (
        ...
        >
          ...
          <div className="w-14 h-14 rounded-2xl bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 flex items-center justify-center mb-4 shadow-sm">
            {isDragActive
              ? <UploadCloud size={26} className="text-[#1e51f9]" />
