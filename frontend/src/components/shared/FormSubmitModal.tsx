...
import { uploadFile, getFileViewUrl } from '@/lib/appwrite/api';
...
const FormSubmitModal = ({ template, onClose, onSuccess }: Props) => {
  ...
  const searchApprover = (q: string) => {
    ...
    searchRef.current = setTimeout(async () => {
      setIsSearching(true);
      try {
        const res = await fetch(`${BACKEND}/api/users/search?q=${encodeURIComponent(q)}`, {
          credentials: 'include',
        });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const uploadedFileUrl = getFileViewUrl(uploaded.$id);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-md">
        ...
        <div className="px-5 py-4 flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600">
            ...
            <div className="min-w-0">
              ...
              <a href={template.fileUrl} download={template.fileName}
                className="text-xs text-[#0068FF] hover:underline font-medium">
                Tải template gốc ({template.fileName})
              </a>
            </div>
          </div>
          ...
          <div>
            ...
            <label className={`flex items-center gap-3 cursor-pointer px-3 py-2.5 rounded-lg border-2 border-dashed transition-colors ${
              file ? 'border-[#0068FF]/40 bg-[#0068FF]/4' : 'border-slate-200 dark:border-slate-600 hover:border-[#0068FF]/40'
            }`}>
              <Upload size={15} className={file ? 'text-[#0068FF]' : 'text-slate-400'} />
              <span className="text-sm text-slate-600 dark:text-slate-300 truncate">
                {file ? file.name : 'Chọn file (PDF, DOCX, ...)'}
              </span>
              {file && <Check size={14} className="text-[#0068FF] shrink-0 ml-auto" />}
          ...
          <div ref={wrapperRef} className="relative">
            ...
            <div className="relative">
              ...
              {approverName && !isSearching && (
                <Check size={12} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#0068FF]" />
              )}
            </div>
            ...
            {approverName && (
              <p className="text-xs text-[#0068FF] mt-1 flex items-center gap-1">
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t border-slate-100 dark:border-slate-700">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-xs text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
            Hủy
          </button>
          <button
          ...
          >
            ...
            {isSubmitting ? 'Đang nộp...' : 'Nộp biểu mẫu'}
          </button>
        </div>
      </div>
    </div>
