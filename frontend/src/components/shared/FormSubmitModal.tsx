...
const inputCls = 'w-full px-3 py-2 text-sm rounded-lg bg-slate-50 dark:bg-slate-700/60 border border-slate-200 dark:border-slate-600 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#F15A22]/25 focus:border-[#F15A22] transition-all';
  ...
  const buildApproverEmail = (reviewUrl: string) => `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#f8fafc;padding:20px">
      <div style="background:white;border-radius:12px;padding:28px;border:1px solid #e2e8f0">
        ...
        <a href="${reviewUrl}" style="display:inline-block;background:#F15A22;color:white;text-decoration:none;padding:10px 24px;border-radius:8px;font-size:13px;font-weight:600">
          Xem &amp; Duyệt biểu mẫu →
        </a>
        ...
        <div className="px-5 py-4 flex flex-col gap-4">
          ...
          <div>
            ...
            <label className={`flex items-center gap-3 cursor-pointer px-3 py-2.5 rounded-lg border-2 border-dashed transition-colors ${
              file ? 'border-[#F15A22]/40 bg-[#F15A22]/4' : 'border-slate-200 dark:border-slate-600 hover:border-[#F15A22]/40'
            }`}>
              <Upload size={15} className={file ? 'text-[#F15A22]' : 'text-slate-400'} />
              <span className="text-sm text-slate-600 dark:text-slate-300 truncate flex-1">
                {file ? file.name : 'Chọn file (PDF, DOCX, …)'}
              </span>
              {file && <Check size={14} className="text-[#F15A22] shrink-0" />}
              <input
                type="file"
                className="hidden"
          ...
          <div ref={wrapperRef} className="relative">
            ...
            <div className="relative">
              ...
              {approverName && !isSearching && (
                <Check size={12} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#F15A22]" />
              )}
            </div>
            ...
            {approverName && (
              <p className="text-xs text-[#F15A22] mt-1 flex items-center gap-1">
