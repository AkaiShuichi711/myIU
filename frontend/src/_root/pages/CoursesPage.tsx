...
const CourseCard = ({ course, role }: { course: any; role: 'lecturer' | 'student' }) => {
  const coverColor = course.coverColor ?? '#0057A8';
  return (
    <Link
    ...
    >
      ...
      <div className="p-3.5">
        <h3 className="font-semibold text-slate-800 dark:text-slate-100 text-sm leading-snug line-clamp-2 mb-1 group-hover:text-[#0085b3] transition-colors">
          {course.name}
        </h3>
        ...
        <div className="flex items-center justify-between">
          ...
          <ChevronRight size={13} className="text-slate-300 dark:text-slate-600 group-hover:text-[#0085b3] transition-colors" />
        </div>
      </div>
    </Link>
  );
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2.5">
                <BookOpen size={16} className="text-[#0057A8]" />
              ...
              {TAB_DEFS.map(({ id, label, icon: Icon, count }) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-md text-xs font-semibold transition-all ${
                    tab === id
                      ? 'bg-white dark:bg-slate-700 text-[#0057A8] shadow-sm'
          ...
          tab === 'teaching' ? (
            <div className="flex flex-col items-center py-20 gap-3 text-center">
              <div className="w-14 h-14 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex items-center justify-center">
                <Briefcase size={24} className="text-[#0057A8]/60" />
              </div>
              ...
              <Link
                to="/courses/create"
                className="mt-1 px-4 py-2 rounded-lg text-xs font-semibold text-white bg-[#0057A8] hover:bg-[#0087b3] transition-colors"
              >
            ...
            <div className="flex flex-col items-center py-16 gap-4 text-center max-w-sm mx-auto">
              ...
              <div className="w-full bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl p-4 text-left">
                <p className="text-[11px] font-bold text-[#0057A8] uppercase tracking-wider mb-2 flex items-center gap-1.5">
                ...
                <ol className="flex flex-col gap-1.5 text-xs text-slate-600 dark:text-slate-400">
                  {['Giảng viên tạo môn học và nhóm học', 'GV thêm sinh viên vào nhóm bằng User ID', 'Môn học xuất hiện ở đây ngay lập tức'].map((step, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <span className="w-4 h-4 rounded-full border border-[#0057A8]/40 text-[#0057A8] flex items-center justify-center text-[9px] font-bold shrink-0 mt-0.5">{i + 1}</span>
                      {step}
                    </li>
                  ))}
                </ol>
