...
const CourseCard = ({ course, role }: { course: any; role: 'lecturer' | 'student' }) => {
  const coverColor = course.coverColor ?? '#F15A22';
  return (
    <Link
    ...
    >
      ...
      <div className="p-3.5">
        <h3 className="font-semibold text-slate-800 dark:text-slate-100 text-sm leading-snug line-clamp-2 mb-1 group-hover:text-[#F15A22] transition-colors">
          {course.name}
        </h3>
        ...
        <div className="flex items-center justify-between">
          ...
          <ChevronRight size={13} className="text-slate-300 dark:text-slate-600 group-hover:text-[#F15A22] transition-colors" />
        </div>
      </div>
    </Link>
  );
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2.5">
                <BookOpen size={16} className="text-[#F15A22]" />
              ...
              {TAB_DEFS.map(({ id, label, icon: Icon, count }) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-md text-xs font-semibold transition-all ${
                    tab === id
                      ? 'bg-white dark:bg-slate-700 text-[#F15A22] shadow-sm'
        ...
        {isLoading ? (
          ...
          tab === 'teaching' ? (
            <div className="flex flex-col items-center py-20 gap-3 text-center">
              <div className="w-14 h-14 rounded-xl border border-[#F15A22]/20 bg-[#F15A22]/5 flex items-center justify-center">
                <Briefcase size={24} className="text-[#F15A22]/60" />
              </div>
              ...
              <Link
                to="/courses/create"
                className="mt-1 px-4 py-2 rounded-lg text-xs font-semibold text-white bg-[#F15A22] hover:bg-[#0087b3] transition-colors"
              >
            ...
            <div className="flex flex-col items-center py-16 gap-4 text-center max-w-sm mx-auto">
              ...
              <div className="w-full bg-[#F15A22]/5 dark:bg-[#F15A22]/8 border border-[#F15A22]/15 rounded-xl p-4 text-left">
                <p className="text-[11px] font-bold text-[#F15A22] uppercase tracking-wider mb-2 flex items-center gap-1.5">
                ...
                <ol className="flex flex-col gap-1.5 text-xs text-slate-600 dark:text-slate-400">
                  {['Giảng viên tạo môn học và nhóm học', 'GV thêm sinh viên vào nhóm bằng User ID', 'Môn học xuất hiện ở đây ngay lập tức'].map((step, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <span className="w-4 h-4 rounded-full bg-[#F15A22]/15 text-[#F15A22] flex items-center justify-center text-[9px] font-bold shrink-0 mt-0.5">{i + 1}</span>
                      {step}
                    </li>
                  ))}
                </ol>
